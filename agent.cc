...
void Agent::PlanIteration(ThreadPool* pool) {
  ...
  // plan
  if (!allocate_enabled) { 
    // === DEBUG: stampa stato ogni 20 step ===
bool visualiza_state_debug_enabled=false;
if(visualiza_state_debug_enabled)
{
    ...
    if (debug_counter % 20 == 0) {
        ...
        std::cout << "\n===== STATE DEBUG (step " << debug_counter << ") =====\n";
...
/*
// ---- Override stato prima del planner ----
// ---- Override stato prima del planner ----

// 1) Accedi allo stato interno
const auto& x_in = state.state();
int nq = model_->nq;
int nv = model_->nv;

// 2) Copia stato + mocap + userdata
static thread_local std::vector<double> x; 
x = x_in;  // copia completa qpos+qvel+act

std::vector<double> mocap = state.mocap();
std::vector<double> userdata = state.userdata();

// 3) Modifica qpos: metti a zero i primi 2 valori
if (nq >= 2) {
    x[0] = 0.0;
    x[1] = 0.0;
    x[2] = 0.0;  // opzionale: yaw per humanoid
    x[3] = 1.0;  // opzionale: w del quaternion
    x[4] = 0.0;  // opzionale: x del quaternion
    x[5] = 0.0;  // opzionale: y del quaternion
    x[6] = 0.0;  // opzionale: z del quaternion
    x[nq + 0] = 0.0; // azzera anche velocità iniziali
    x[nq + 1] = 0.0;
    x[nq + 2] = 0.0;
    x[nq + 3] = 0.0;
    x[nq + 4] = 0.0;
...
// 4) Ricava puntatori
const double* qpos = x.data();
const double* qvel = x.data() + nq;
const double* act  = x.data() + nq + nv;
const double* mocap_pos  = mocap.data();
const double* mocap_quat = mocap.data() + 3 * model_->nmocap;
const double* usr = userdata.data();

double t = state.time();

// 5) Scrivi il nuovo stato nel planner
state.Set(model_, qpos, qvel, act,
          mocap_pos, mocap_quat, usr, t);
...
void Agent::Plots(const mjData* data, int shift) {
  ...
  // ----- record residuals to file ----- //
bool record_residuals=false;
if (record_residuals)
{
  const std::string filename = "../mjpc/tasks/h1_2/dati/residui.txt";

  static const std::vector<std::pair<std::string, int>> groups = {
      {"Height", 1}, {"PelvisFeet", 1}, {"Balance", 2}, {"Upright", 8},
      {"Posture", 27}, {"FaceForward", 2}, {"Velocity", 1}, {"move feet", 2},
      {"Control", 27}, {"Clearance", 1}, {"FeetDistance", 1}, {"XXX", 1}};

  // Buffer 
  static std::vector<std::vector<double>> buffer;  
  static bool header_written = false;
  static bool file_cleared = false;

  const double* r = data->sensordata;
  double t = data->time;

  std::vector<double> row;
  row.reserve(groups.size() + 1);
  row.push_back(t);

  int idx = 0;
  for (auto& g : groups) {
    double s = 0.0;
    for (int k = 0; k < g.second; k++) s += r[idx + k];
    row.push_back(s);
    idx += g.second;
  }
  ...
  if (buffer.size() >= 100) {
    ...
    if (!file_cleared) {
      residual_file.open(filename, std::ios::trunc);
      file_cleared = true;
    } else {
      residual_file.open(filename, std::ios::app);
    }
    ...
    if (!residual_file.is_open()) {
      ...
      if (!header_written) {
        residual_file << "time";
        for (auto& g : groups) residual_file << "," << g.first;
        residual_file << "\n";
        header_written = true;
      }

      // buffer
      for (const auto& riga : buffer) {
        residual_file << std::fixed << std::setprecision(6) << riga[0];
        for (size_t j = 1; j < riga.size(); j++)
          residual_file << "," << riga[j];
        residual_file << "\n";
      }
