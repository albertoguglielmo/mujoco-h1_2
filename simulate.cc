...
double desired_clearance_obstacle = 0.1;
double obstacle = 1e9;
...
void UpdateWatch(mj::Simulate* sim) {
    ...
    // macro necessarie per accedere ai campi di mjData
    MJDATA_POINTERS_PREAMBLE(sim->m);

    // cerca il campo specificato in watch
    #define X(TYPE, NAME, NR, NC)                                                                  \
      if (!mju::strcmp_arr(#NAME, sim->field) &&                                                   \
        ...
        /* ritorno per evitare interferenze con logging */                                         \
        goto log_sensors;                                                                          \
      }
...
log_sensors:
    if (record){
        // 2= visione dinamica per il task integrato
        // 1= salva i valori giunti
        ...
        if (integrated_view){
          ...
          const mjData* data = sim->d;
          auto point_matrix = MapPointCloudMatrix(model, data);
          std::cout << "\n--- Point with maximum Z (Pz > 0.01) ---\n";

          Point max_point = {0.0, 0.0, -1e9};  
          bool found = false;
          
          for (size_t i = 0; i < point_matrix.size(); ++i) {
              for (size_t j = 0; j < point_matrix[i].size(); ++j) {
                  const auto& p = point_matrix[i][j];
                  if (!std::isnan(p.x) && p.z > 0.01) {
                      if (p.z > max_point.z) {
                      max_point.x = p.x;
                      max_point.y = p.y;
                      max_point.z = p.z;
          ...
          if (found) {
              printf("Punto con Z massima: X=%.3f, Y=%.3f, Z=%.3f\n", max_point.x, max_point.y, max_point.z);
                    if (desired_clearance_obstacle<max_point.z+0.18 && max_point.z<0.4 ){
                      desired_clearance_obstacle= max_point.z+0.18;
          ...
          if(foot_left[0]>obstacle && foot_right[0]>obstacle){
            desired_clearance_obstacle=0.1;
            obstacle=1e9;
          }

          printf("desired_clearance_obstacle: %.3f\n", desired_clearance_obstacle);
          printf("obstacle updated: %.3f\n", obstacle);

          std::ofstream outfile("../mjpc/tasks/h1_2/dati/desired_clearance_obstacle.txt", std::ios::app);
          if (outfile.is_open()) {
              outfile << data->time << ","
                      << desired_clearance_obstacle << ","
                      << max_point.x << ","
              ...
              std::cerr << " Error: Unable to open the file desired_clearance_obstacle.txt" << std::endl;
          }

          }
        if (record_point_cloud){ 
          
            const mjModel* model = sim->m;
            const mjData* data = sim->d;
            auto point_matrix = MapPointCloudMatrix(model, data);
            std::ofstream outfile("../mjpc/tasks/h1_2/dati/punti.txt", std::ios::app);
        ...
        if (record_joint) {
            ...
            const std::string filename = "../mjpc/tasks/h1_2/dati/giunti.txt";
            static bool file_cleared = false;
        ...
        if (record_data) {
          ...
          if (!foot_right || !foot_left || !torso_velocity ||
              !torso_forward || !pelvis_forward || !foot_right_forward || !foot_left_forward || !subcom) {
              std::cerr << "Errore: sensori mancanti!\n";
              return;
          }
          ...
          double angle_rad = atan2(forward[1], forward[0]);
          double angle_deg = angle_rad * (180.0 / M_PI);
          double angle_deg_goal = 0.0;     
          double speed_goal = 1.0;         

          // === Compute Euclidean norm of torso velocity ===
          double torso_speed_norm = std::sqrt(
              torso_velocity[0] * torso_velocity[0] +
              torso_velocity[1] * torso_velocity[1]);

          // --- Perpendicular distance CoM - line between the feet ---
          double dx = foot_right[0] - foot_left[0];
          double dy = foot_right[1] - foot_left[1];
          double num = fabs(dy * (subcom[0] - foot_left[0]) - dx * (subcom[1] - foot_left[1]));
          double den = sqrt(dx * dx + dy * dy);
          double dist_cdm_line = (den > 1e-8) ? num / den : 0.0;

          // === Buffer for saving data ===
          const std::string filename = "../mjpc/tasks/h1_2/dati/dati.txt";
          ...
          struct Sample {
              ...
              double torso_speed_norm;
              double speed_goal;
              double angle_deg;
              double angle_deg_goal;
              double dist_cdm_line;
          };
          ...
          // === 1) Add current sample to buffer ===
          buffer.push_back({
              data->time,
              foot_right[0], foot_right[1], foot_right[2],
              foot_left[0],  foot_left[1],  foot_left[2],
              subcom[0], subcom[1], subcom[2],
              torso_speed_norm, speed_goal, angle_deg, angle_deg_goal, dist_cdm_line
          });

          // === 2) Write to file every 100 samples ===
          if (buffer.size() >= 100) {
              std::ofstream out;
              ...
              if (!header_written) {
                  out << "time,"
                      ...
                      << "CoM_x,CoM_y,CoM_z,"
                      << "torso_speed_norm,speed_goal,angle_deg,angle_deg_goal,"
                      << "dist_cdm_line\n";
                  header_written = true;
              }
