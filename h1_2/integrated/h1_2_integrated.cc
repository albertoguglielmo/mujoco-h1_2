...
extern double desired_clearance_obstacle;


namespace mjpc {

std::string H1_2_integrated_Task::XmlPath() const {
  return GetModelPath("h1_2/h1_2_integrated.xml");
}
std::string H1_2_integrated_Task::Name() const { return "h1_2_integrated"; }
...
void H1_2_integrated_Task::ResidualFn::Residual(const mjModel *model, const mjData *data,
  ...
  const double posizione[2] = {2, 0}; //move up to end fuselage

  // distance to current target
  double dx_dir = posizione[0] - torso_position[0];
  double dy_dir = posizione[1] - torso_position[1];
  double const direction_goal = atan2(dy_dir, dx_dir) * 180.0 / M_PI;
  double const torso_height_goal = parameters_[0];
  double const speed_goal = parameters_[1];
  double const feet_distance_goal = parameters_[2];
  double desired_clearance =  parameters_[3];
  ...
  // mask: 27 elements (encourages movements of the hip_pitch_joint and right_knee_joint)
  ...
  // com vel
  double *waist_lower_subcomvel = SensorByName(model, data, "waist_lower_subcomvel");
  ...
  //PLEASE NOTE THAT IN ORDER FOR OBSTACLE DETECTION TO WORK CORRECTLY, YOU MUST ENABLE DYNAMIC VISION IN /mjpc/simulate.cc BY SETTING Record_what == 2.
  if (foot_right[0]>(obstacle-0.3) && foot_right[0]<(obstacle+0.45)){
  
    residual[counter++] = 4*(abs(desired_clearance_obstacle -fabs((foot_left[2]) - (foot_right[2])))+ abs(desired_clearance_obstacle -(foot_left[2]) - (foot_right[2])));}
  else{
