...
std::string H1_2_walk_Task2::XmlPath() const {
  return GetModelPath("h1_2_walk2/h1_2.xml");
}
std::string H1_2_walk_Task2::Name() const { return "h1_2_walk2"; }
...
// ----------------------------------------------------------------
void H1_2_walk_Task2::ResidualFn::Residual(const mjModel *model, const mjData *data,
  ...
  double const desired_clearance =  parameters_[4];
  ...
  // penalizza se clearance è minore del goal
  
  double clearance = mju_max(foot_left[2], foot_right[2]);
  residual[counter++] = desired_clearance - mju_max(clearance, 0.0);
...
std::ofstream outfile("../mjpc/tasks/h1_2_walk2/dati/punti.txt", std::ios::app);


std::cout << "\n--- Stampa Matrice Punti Validi (Pz > 0.01) ---\n";
...
for (size_t i = 0; i < point_matrix.size(); ++i) {
    for (size_t j = 0; j < point_matrix[i].size(); ++j) {
        ...
        if (!std::isnan(p.x)) {
            ...
            // stampa a video
            printf("Matrice[%2zu][%2zu]: X=%.3f, Y=%.3f, Z=%.3f\n",
                   i, j, p.x, p.y, p.z);
        }
    }
}

std::cout << "--- Fine Stampa ---\n";
